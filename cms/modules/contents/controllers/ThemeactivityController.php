...
class Contents_ThemeactivityController extends BaseController
{
    private $channel_db;
...
/*        // 权限检查
        ...
        $select = $this->channel_db->select()->from('themeactivity');
     ...
     * 活动添加
     */
    public function themeactivityAddAction()
    {
       // $this->_checkPermission('themeactivity', 'themeactivity-add');
        if ($this->_request->isPost()) {
            $activity = array(
                'username' => trim($this->_getParam('username')) ? trim($this->_getParam('username')) : $this->_showMsg('用户名不能为空！'),
                'desc' => trim($this->_getParam('desc')) ? trim($this->_getParam('desc')) : $this->_showMsg('获得奖品不能为空'),
                'getrewardtime' => trim($this->_getParam('getrewardtime')) ? trim($this->_getParam('getrewardtime')) : $this->_showMsg('获奖时间'),
                'create_time' => time(),
            );
            $this->channel_db->insert('themeactivity', $activity);
            $this->flash('添加成功！', '/contents/themeactivity/index', 1);
        }
    }
    ...
    public function activityEditAction()
    {
        ...
        if ($this->_request->isPost()) {
            $activity = array(
                ...
                'base_score' => intval(trim($this->_getParam('base_score'))) ? intval(trim($this->_getParam('base_score'))) : $this->_showMsg('基础分数填写错误！'),
            );

            // 获奖规则
            // 按尾号判定是否得奖
            $award_tail_num = intval(trim($this->_getParam('award_tail_num')));
            if ($award_tail_num) {
     ...
     * 活动删除
     */
    public function themeactivityDelAction()
    {
       // $this->_checkPermission('activity', 'activity-del');
        $id = intval($this->_getParam('id')) ? intval($this->_getParam('id')) : $this->_showMsg('没有指定活动ID');
        $this->channel_db->delete('themeactivity', '`id` = ' . $id);
        $this->flash('删除成功！', '/contents/themeactivity/index', 2);
    }
